...
interface KaeuferViewDialogProps {
  open: boolean;
  onClose: () => void;
  record: Kaeufer | null;
  onEdit: (record: Kaeufer) => void;
}

export function KaeuferViewDialog({ open, onClose, record, onEdit }: KaeuferViewDialogProps) {
  ...
  return (
    <Dialog open={open} onOpenChange={v => !v && onClose()}>
      <DialogContent className="max-w-lg max-h-[90vh] overflow-y-auto">
        <DialogHeader>
          <DialogTitle>Käufer anzeigen</DialogTitle>
        </DialogHeader>
        ...
        <div className="space-y-4">
          ...
          <div className="space-y-1">
            <Label className="text-xs text-muted-foreground">Registrierungsdatum</Label>
            <p className="text-sm">{formatDate(record.fields.registrierungsdatum)}</p>
          </div>
        </div>
      </DialogContent>
    </Dialog>
