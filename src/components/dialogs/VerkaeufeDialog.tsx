...
export function VerkaeufeDialog({ open, onClose, onSubmit, defaultValues, artikelList, kaeuferList, enablePhotoScan = false, enablePhotoLocation = true }: VerkaeufeDialogProps) {
  ...
  async function handleSubmit(e: React.FormEvent) {
    ...
    setSaving(true);
    try {
      const clean = cleanFieldsForApi({ ...fields }, 'verkaeufe');
      await onSubmit(clean as Verkaeufe['fields']);
      onClose();
    } finally {
      setSaving(false);
    }
  ...
  async function handlePhotoScan(file: File) {
    ...
    try {
      ...
      contextParts.push(`<available-records field="kaeufer" entity="Käufer">\n${JSON.stringify(kaeuferList.map(r => ({ record_id: r.record_id, ...r.fields })), null, 2)}\n</available-records>`);
      if (usePersonalInfo) {
        try {
      ...
      const schema = `{\n  "artikel": string | null, // Display name from Artikel (see <available-records>)\n  "verkaufspreis": number | null, // Verkaufspreis (EUR)\n  "verkaufsdatum": string | null, // YYYY-MM-DDTHH:MM\n  "kaeufer": string | null, // Display name from Käufer (see <available-records>)\n  "zahlungsmethode": LookupValue | null, // Zahlungsmethode (select one key: "bargeld" | "ec_karte" | "kreditkarte" | "paypal" | "ueberweisung" | "sonstiges") mapping: bargeld=Bargeld, ec_karte=EC-Karte, kreditkarte=Kreditkarte, paypal=PayPal, ueberweisung=Überweisung, sonstiges=Sonstiges\n  "notizen": string | null, // Notizen\n}`;
  ...
  const DIALOG_INTENT = defaultValues ? 'Verkäufe bearbeiten' : 'Verkäufe hinzufügen';
