...
import { KaeuferViewDialog } from '@/components/dialogs/KaeuferViewDialog';
...
const ARTIKEL_FIELDS = [
  ...
  { key: 'foto', label: 'Foto des Artikels', type: 'file' },
];
const KAEUFER_FIELDS = [
  ...
  { key: 'registrierungsdatum', label: 'Registrierungsdatum', type: 'date/date' },
];
const VERKAEUFER_FIELDS = [
  { key: 'verkaeufernummer', label: 'Verkäufernummer', type: 'string/text' },
  { key: 'vorname', label: 'Vorname', type: 'string/text' },
  { key: 'nachname', label: 'Nachname', type: 'string/text' },
  { key: 'telefon', label: 'Telefonnummer', type: 'string/tel' },
  { key: 'email', label: 'E-Mail-Adresse', type: 'string/email' },
...
const VERKAEUFE_FIELDS = [
  { key: 'artikel', label: 'Verkaufter Artikel', type: 'applookup/select', targetEntity: 'artikel', targetAppId: 'ARTIKEL', displayField: 'artikelnummer' },
  { key: 'verkaufspreis', label: 'Verkaufspreis (EUR)', type: 'number' },
  { key: 'verkaufsdatum', label: 'Verkaufsdatum und -uhrzeit', type: 'date/datetimeminute' },
  { key: 'kaeufer', label: 'Käufer', type: 'applookup/select', targetEntity: 'kaeufer', targetAppId: 'KAEUFER', displayField: 'vorname' },
  { key: 'zahlungsmethode', label: 'Zahlungsmethode', type: 'lookup/select', options: [{ key: 'bargeld', label: 'Bargeld' }, { key: 'ec_karte', label: 'EC-Karte' }, { key: 'kreditkarte', label: 'Kreditkarte' }, { key: 'paypal', label: 'PayPal' }, { key: 'ueberweisung', label: 'Überweisung' }, { key: 'sonstiges', label: 'Sonstiges' }] },
  { key: 'notizen', label: 'Notizen', type: 'string/textarea' },
];

const ENTITY_TABS = [
  { key: 'artikel', label: 'Artikel', pascal: 'Artikel' },
  { key: 'kaeufer', label: 'Käufer', pascal: 'Kaeufer' },
  { key: 'verkaeufer', label: 'Verkäufer', pascal: 'Verkaeufer' },
  { key: 'verkaeufe', label: 'Verkäufe', pascal: 'Verkaeufe' },
] as const;
...
export default function AdminPage() {
  ...
  const getRecords = useCallback((entity: EntityKey) => {
    switch (entity) {
      ...
      case 'kaeufer': return (data as any).kaeufer as Kaeufer[] ?? [];
      case 'verkaeufer': return (data as any).verkaeufer as Verkaeufer[] ?? [];
      case 'verkaeufe': return (data as any).verkaeufe as Verkaeufe[] ?? [];
      default: return [];
    }
  }, [data]);
  ...
  const getLookupLists = useCallback((entity: EntityKey) => {
    ...
    switch (entity) {
      ...
      case 'verkaeufe':
        ...
        lists.kaeuferList = (data as any).kaeufer ?? [];
        break;
    }
    return lists;
  }, [data]);
  ...
  const getApplookupDisplay = useCallback((entity: EntityKey, fieldKey: string, url?: unknown) => {
    ...
    if (entity === 'artikel' && fieldKey === 'verkaeufer') {
      ...
      return match?.fields.verkaeufernummer ?? '—';
    }
    if (entity === 'verkaeufe' && fieldKey === 'artikel') {
      const match = (lists.artikelList ?? []).find((r: any) => r.record_id === id);
      return match?.fields.artikelnummer ?? '—';
    }
    if (entity === 'verkaeufe' && fieldKey === 'kaeufer') {
      const match = (lists.kaeuferList ?? []).find((r: any) => r.record_id === id);
  ...
  const getFieldMeta = useCallback((entity: EntityKey) => {
    switch (entity) {
      ...
      case 'kaeufer': return KAEUFER_FIELDS;
      case 'verkaeufer': return VERKAEUFER_FIELDS;
      case 'verkaeufe': return VERKAEUFE_FIELDS;
      default: return [];
    }
  }, []);
  ...
  const getServiceMethods = useCallback((entity: EntityKey) => {
    switch (entity) {
      ...
      case 'kaeufer': return {
        create: (fields: any) => LivingAppsService.createKaeuferEntry(fields),
        update: (id: string, fields: any) => LivingAppsService.updateKaeuferEntry(id, fields),
        remove: (id: string) => LivingAppsService.deleteKaeuferEntry(id),
      };
      ...
      case 'verkaeufe': return {
        create: (fields: any) => LivingAppsService.createVerkaeufeEntry(fields),
        update: (id: string, fields: any) => LivingAppsService.updateVerkaeufeEntry(id, fields),
        remove: (id: string) => LivingAppsService.deleteVerkaeufeEntry(id),
      };
      default: return null;
    }
  }, []);
  ...
  return (
    ...
    >
      ...
      {(createEntity === 'kaeufer' || dialogState?.entity === 'kaeufer') && (
        <KaeuferDialog
          open={createEntity === 'kaeufer' || dialogState?.entity === 'kaeufer'}
          onClose={() => { setCreateEntity(null); setDialogState(null); }}
          onSubmit={dialogState?.entity === 'kaeufer' ? handleUpdate : (fields: any) => handleCreate('kaeufer', fields)}
          defaultValues={dialogState?.entity === 'kaeufer' ? dialogState.record?.fields : undefined}
          enablePhotoScan={AI_PHOTO_SCAN['Kaeufer']}
          enablePhotoLocation={AI_PHOTO_LOCATION['Kaeufer']}
        />
      )}
      ...
      {(createEntity === 'verkaeufe' || dialogState?.entity === 'verkaeufe') && (
        <VerkaeufeDialog
          open={createEntity === 'verkaeufe' || dialogState?.entity === 'verkaeufe'}
          onClose={() => { setCreateEntity(null); setDialogState(null); }}
          onSubmit={dialogState?.entity === 'verkaeufe' ? handleUpdate : (fields: any) => handleCreate('verkaeufe', fields)}
          defaultValues={dialogState?.entity === 'verkaeufe' ? dialogState.record?.fields : undefined}
          artikelList={(data as any).artikel ?? []}
          kaeuferList={(data as any).kaeufer ?? []}
      ...
      {viewState?.entity === 'kaeufer' && (
        <KaeuferViewDialog
          open={viewState?.entity === 'kaeufer'}
          onClose={() => setViewState(null)}
          record={viewState?.record}
          onEdit={(r: any) => { setViewState(null); setDialogState({ entity: 'kaeufer', record: r }); }}
        />
      )}
      ...
      {viewState?.entity === 'verkaeufe' && (
        <VerkaeufeViewDialog
          open={viewState?.entity === 'verkaeufe'}
          onClose={() => setViewState(null)}
          record={viewState?.record}
          onEdit={(r: any) => { setViewState(null); setDialogState({ entity: 'verkaeufe', record: r }); }}
          artikelList={(data as any).artikel ?? []}
          kaeuferList={(data as any).kaeufer ?? []}
        />
      )}
